<?php
$btn = array(1=>'btn-warning',5=>'btn-success',6=>'btn-danger' );
?>
<table class="table mc-read-table table-responsive" id="table-read">
    ...
    <tbody>
        ...
        <tr  rel="<?=$register->id;?>"  class="sin_leer_<?=$register->estado_id?>">
            <td><?=$register->nombre.' '.$register->apellido;?></td>
            <td><?=$register->detalle;?></td>
            <td><?=get_fecha($register->fecha);?></td>
            <td>
                <a href="<?=base_url();?>inquilinos/consultas_view/<?=$register->id;?>" class="btn <?=$btn[$register->estado_id]?> btn-xs" title="Ver Consultas"><i class="glyphicon glyphicon-eye-open"></i> Ver
                </a>
            </td>
        </tr>
        <?php } ?>
