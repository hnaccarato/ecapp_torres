...
class Ion_auth
{
      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				else
				{
					...
					$this->email->clear();
					$this->email->from(MAIL_INFO, $this->config->item('site_title', 'ion_auth'));
	...
	public function set_password($identity)    //changed $email to $identity
	{
		//return TRUE;
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				{
					$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('set_password', 'ion_auth'), $data, true);
					$this->email->clear();
					$this->email->from(MAIL_INFO, 'Building Apps');
					$this->email->to($user->email);
					$this->email->subject('Bienvenido al sistema');
					$this->email->message($message);
