...
class Administrador extends CI_Controller {
	...
	public function __construct(){
		...
		$this->layout->setFolder('administrador');
		$this->layout->setLayout('administrador/layout');
		...
		$this->session->set_userdata(array('controller'=>ADMINISTRADOR));
		...
		$this->espacios_model->set_rol(ADMINISTRADOR);
	...
	public function edificios_update(){
		...
		if(!empty($_POST)){
			...
			if(!empty($_FILES['imagen']['name'])){
				...
				$filename =  $name_file.'.'.strtolower($file_data['extension']);
			...
			if(!$this->edificios_model->update($data, $where)){
				redirect('administrador/edificios_error');
			}
...
//echo $this->db->last_query();
	$this->load->view('administrador/recibos/read', $data);
}
	...
	try {
		...
		if(!empty($_POST)){
			...
			if(!empty($_FILES['file']['name'])){
				...
				'_expensa_'.'.'.strtolower($file_data['extension']);
			...
			if(!empty($_FILES['prorrateo']['name'])){
				...
				'.'.strtolower($file_data['extension']);
				$config['file_name'] = $filename;
			...
			if(!$this->recibos_model->create($data)){
				redirect('administrador/recibos_error');
			}
			...
			redirect('administrador/expensas_list/');
		}
	    ...
	    echo 'Excepción expensas create: ',  $e->getMessage(), "\n";
	}
}
...
public function expensas_update($primary_key_value){
	...
	if(!empty($_FILES['file']['name'])){
		...
		$config['upload_path'] = UPLOAD_EXPENSAS;
		$config['allowed_types'] = 'xlsx|csv|docx|xls|pdf|txt|doc|zip';
		$file = $_FILES['file']['name'];
		$file_data = pathinfo($file);
	...
	if(!empty($_FILES['gparticulares']['name'])){
		...
		if ($estado_id == 9) {
			$update['estado_id'] =  ENVIADO;
		}else{
			if($_POST['estado_id'] > 0){
				$update['estado_id'] = $estado_id;
			}
		}
		...
		redirect('administrador/expensas_list/'.$data['values']->tipo_recibos);
	}
...
public function notificar($recibo_id){
	...
	if(isset($_POST['unidad_id'])){
		...
		foreach($unidades as $key=>$value) {
			$email  =	$this->unidades_model->get_email($value);
		}
...
public function pagos_create(){
	...
	if(!empty($_POST)){
		...
		redirect('administrador/expensas_pagos_list');
	}
...
//		$this->db->where('pagos_users.active',TRUE);	
	...
	$this->load->view('administrador/pagos/read', $data);
}
...
public function pagos_delete($primary_key_value){
	...
	if(!$this->pagos_model->delete($where)){
		redirect('administrador/pagos_error');
	}
	redirect('administrador/expensas_pagos_list');
}
...
public function view_pagos(){
	...
	$this->db->where_in('estados.id',array(9,11,10,1,3));
	$data['estados'] = $this->db->get('estados')->result();
	echo $this->load->view('administrador/pagos/view',$data,TRUE);
}
...
public function unidad_create(){
	...
	if(!empty($_POST)){
		...
		if(!empty($_FILES['base_unidades']['name'])){
			...
			if($this->upload($config,'base_unidades')){ 
				...
				if (($fichero = fopen($config['upload_path'].$filename, "r")) !== FALSE) {
					...
					/*** para update de unidades descomentar y comentar el create
					 * 
					 * 		
					 * $where = array('edificio_id'=>$this->edificio_id,'name'=>$datos[0]);
					 * $this->unidades_model->update(array('name'=>$datos[1]),$where);
					 * $unidad_id = false;
					 * 
					 * 
					 */
						...
						if($unidad_id){
							...
							if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
								$name = trim($datos[5]);
								$password = 'Yacht2024';
								$this->create_user($email,$password,$name,$this->edificio_id,$unidad_id);
							}else{
		...
		redirect('administrador/unidad_list');
	}
...
public function unidad_read(){
	...
	$this->load->view('administrador/unidades/read', $data);
}
...
public function propietarios_create(){
	...
	if ($this->form_validation->run() == true)
	{
		...
		if($user_id = $this->check_email($email)){
			...
			redirect('administrador/propietarios_list');
			return TRUE;
		}
...
public function propietarios_read(){
	...
	//	echo $this->db->last_query();
	$this->load->view('administrador/propietarios/read', $data);
...
*/
		...
		if(!$this->ion_auth->update($primary_key_value, $data)){
			redirect('administrador/users_error');
		}
...
/*************************************** inquilinos **********************************************/
    public function inquilinos($user_id){
     ...
     public function inquilinos_read($user_id){
   		...
   		//echo $this->db->last_query();
   		$this->load->view('administrador/inquilinos/read', $data);
	        ...
	        if ($this->form_validation->run() == true)
	        {
				...
				$this->users_model->delete_inquilino_byunidad($unidad_id);
	          	...
	          	if($this->add_inquilino_existent($email,$unidad_id))
	            	redirect('administrador/inquilinos/'.$user_id);
	    		   ...
	    		   if($this->ion_auth->update($user->id, $data))
	    		    {
	    			    ...
	    			    $this->session->set_flashdata('message', $this->ion_auth->messages() );
	    			    redirect('administrador/inquilinos/'.$id);
	    ...
	    public function inquilino_delete($users_unidad_id){
	    	...
	    	if($rs->num_rows()){
	    		...
	    		redirect('administrador/inquilinos');
	    	}else{
	    		redirect('administrador/inquilinos');
	    	}
...
public function consultas_create($user_id){
	...
	if(!empty($_POST)){
		...
		if(!$this->consultas_model->create($data)){
			redirect('administrador/consultas_error');
		}

		redirect('administrador/consultas_list');
	}
...
public function consultas_read(){
	...
	$this->load->view('administrador/consultas/read', $data);
}
...
public function circular_create(){
	...
	if(!empty($_POST)){
		...
		if(!$this->circular_model->create($data)){
			redirect('administrador/circular_error');
		}
		...
		redirect('administrador/circular_list');
	}
...
public function circular_read(){
	...
	$this->load->view('administrador/circular/read', $data);
}
...
public function view_circular_read($circular_id){
	...
	$this->load->view('administrador/circular/read_view', $data);
}
...
public function propuestas_create(){
	...
	if(!empty($_POST)){
		...
		if(!$id_propuesta = $this->propuestas_model->create($data)){
			redirect('administrador/propuestas_error');
		}

		$this->add_opciones($id_propuesta);
		$this->add_archivos_propuesta($id_propuesta);

		redirect('administrador/propuestas_list');
	}
...
public function propuestas_read(){
	...
	//echo $this->db->last_query();
	$this->load->view('administrador/propuestas/read', $data);
}
...
public function asamblea_create(){
	...
	if(!empty($_POST)){
		...
		if(!empty($_FILES['acta_other']['name'])){

			$config['upload_path'] = BASEPATH.'../upload/asamblea/';
			$config['allowed_types'] = 'xlsx|txt|doc|xls|docx|pdf|gif|jpg|png|jpeg';
			$file = $_FILES['acta_other']['name'];
			$file_data = pathinfo($file);
			$name_file = $this->toAscii($file_data['filename']);
			$filename =  $name_file.'.'.strtolower($file_data['extension']);
			$config['file_name'] = $filename;

			if($this->upload($config,'acta_other')){ 
				$data['acta_other'] = $filename;
			}else{
				die("error al subir el archivo");
			}
		}
		...
		if(!$this->asambleas_model->create($data)){
			redirect('administrador/asamblea_error');

		}
		
		redirect('administrador/asamblea_list');
	}
...
public function asamblea_read(){
	...
	$this->load->view('administrador/asamblea/read', $data);
}
...
public function encargado_create(){
	...
	if(!empty($_POST)){
		...
		if(!$this->encargado_model->create($data)){
			redirect('administrador/encargado_error');
		}

		redirect('administrador/encargado_list');
	}
...
public function encargado_read(){
	...
	$this->load->view('administrador/encargado/read', $data);
}
...
public function legales_create(){
	...
	if(!empty($_POST)){
		...
		if(!$id = $this->Legales_model->create($parents)){
			...
			redirect('administrador/legales_error');
		}else{
			    ...
			    {           
			        ...
			        if(!empty($file)){
			            ...
			            if($this->upload->do_upload('file')){
			                ...
			                if ($i == 0) {
			                	$parameter['file'] = $files['file']['name'][$i];
			                	$this->Legales_model->update($parameter, ['id'=>$id]);
			                }
			            }else{
		...
		redirect('administrador/legales_list/'.$legal_id);
	}

	$data['title'] = "Nuevo archivo de ".$rs->name;
	$data['legal_id'] = $legal_id;
	$tokens = $this->users_model->get_all_push_movile($this->edificio_id);
	$this->send_notification($data['title'],"Nuevo archivo cargado" , $tokens);
...
public function legales_update($primary_key_value){
	...
	if(!empty($_POST)){
		...
		if(!empty($_FILES['file']['name'])){
		    ...
		    $uploadPath = BASEPATH.'../upload/legales/';
		    $config['upload_path'] = $uploadPath;
		    $config['allowed_types'] = 'xlsx|txt|doc|xls|docx|pdf|gif|jpg|png|jpeg|html|htmlx';
		    $files = $_FILES;
		    $cpt = count($_FILES['file']['name']);
		...
		$this->db->select('name,file');
		$file = $this->db->get_where('file_legales',array('legal_id'=>$primary_key_value))->row();
		if($file) {
		//	$parents['titulo'] = $file->name;
			$parents['file'] = $file->file;
		}
		...
		redirect('administrador/legales_list/'.$data['values']->tipo_legal_id);
	}

	$data['title'] = "Editar archivo de ".$rs->name;
	$tokens = $this->users_model->get_all_push_movile($this->edificio_id);
	$this->send_notification($data['title'],$data['title'] , $tokens);
	$this->layout->view('legales/update', $data);
}

private function send_notification($title, $body, $tokens)
{
	if(count($tokens) > 0){
		$this->notification->title = $title;
		$this->notification->body =  $body;
		$this->notification->send_push_movile($tokens);
		return TRUE;
	}
...
public function load_file($legal_id){
    ...
    $data['files'] = $rs;
    echo  $this->load->view('administrador/legales/load_files',$data,true);
...
public function legales_read(){
	...
	$data['legal_id'] = $legal_id;
	$this->load->view('administrador/legales/read', $data);
}
...
public function legales_delete($primary_key_value){
	...
	redirect('administrador/legales_list');
...
public function reglamentos_create(){
	...
	if(!empty($_POST)){
		...
		if(!$this->reglamentos_model->create($data)){
			redirect('administrador/reglamentos_error');
		}

		redirect('administrador/reglamentos_list');
	}
...
public function reglamentos_read(){
	...
	$this->load->view('administrador/reglamentos/read', $data);
}
...
public function espacios_create(){
	...
	if($this->form_validation->run()){
		...
		redirect('administrador/espacios_list');
...
public function espacios_update($espacio_id){
	...
	if($this->form_validation->run()){
		...
		if(!$this->espacios_model->update($data, $where)){
			redirect('administrador/espacios_error');
		}
...
public function espacios_load($espacio_id){
	...
	redirect('administrador/espacios_reservar/'.date('Y').'/'.date('m').'/'.$espacio_id);
}
...
public function espacios_reservar($year = null, $month = null,$espacio_id=null){
	...
	{
		redirect('administrador/espacios_reservar/'.date('Y').'/'.date('m'));
	}
...
//obtenemos la información de las horas de ese día
		...
		{   
			...
			if($periodo->num_rows()){
				...
				$this->load->view("administrador/espacios/ajax/get_periodo",$data);
			}
			elseif($turnos->num_rows()){
				$this->load->view("administrador/espacios/ajax/get_turnos",$data);
			}else{
				echo "Error comunicarce con el administrador";
				return false;
			}
...
public function espacios_read(){
	...
	$this->load->view('administrador/espacios/read',$data);
}
...
public function espacios_delete($primary_key_value){
	...
	public function espacios_informes_read($data){
		$this->rat->log(uri_string(),1);
		$this->load->view('administrador/espacios/informes_read', $data);
	}
	...
	public function load_edificio(){
		...
		echo $this->load->view('administrador/load_edificio.php',$data,true);	
	...
	// redirect them back to the admin page
			...
			$this->edificios_model->add_edificios($id,$edificios);
			$this->session->set_flashdata('message', $this->ion_auth->messages());
			redirect('administrador/seguridad_list');
		}
		else
		{
	// display the create user form
	...
	//	echo $this->db->last_query();
		$this->load->view('administrador/seguridad/read', $data);
	}
...
/*
	...
	public function expensa_create(){
		$this->rat->log(uri_string(),1);
		...
		if(!empty($_FILES)){
			...
			if(!empty($_FILES['base_expensa']['name'])){

				$config['upload_path'] = BASEPATH.'../upload/base/';
				$config['allowed_types'] = 'xlsx|xls|csv';
				$file = $_FILES['base_expensa']['name'];
				$file_data = pathinfo($file);
				...
				if($this->upload($config,'base_expensa')){ 
					...
					if (($fichero = fopen($config['upload_path'].$filename, "r")) !== FALSE) {
						...
						fseek($fichero,0);
						$data_table = [];
						$this->db->simple_query('DELETE FROM `expensas` WHERE `edificio_id` = '.$this->edificio_id);
						
						while (($datos = fgetcsv($fichero,null,$boolean)) !== FALSE) {
							
							$unidad = $this->unidades_model->get_my_unidad($this->edificio_id,trim($datos[0]), trim($datos[1]));
...
/** 
								...
								$data['pagos'] = $datos[3];
								$data['deuda'] = $datos[4];
								$data['interes'] = $datos[5];
								$data['gastos_1'] = $datos[6];
								$data['gastos_2'] = $datos[7];
								$data['gastos_3'] = $datos[8];
								$data['mantenimiento'] = $datos[9];
								$data['bar'] = $datos[10];
								$data['gastos_particulares'] = $datos[11];
*/
								$data['total'] = $datos[2];
								$data['total_2'] = $datos[3];
								$unidad_id = $this->Expensa_model->create($data);
							}else {
								$data_table[] = $datos;
							}
						...
						$this->load_data_table($data_table);
					}
				}
			
			}

			redirect('administrador/expensa_list');
		}

		$this->layout->view('expensa/create');
	}


	public function expensa_read(){
		$this->rat->log(uri_string(),1);
		$limit = (isset($_POST['limit']))? $this->input->post('limit') : 50;
		$order_type = 'DESC';
		$order_by = 'unidades.id';
	...
	public function expensa_list(){
		$this->rat->log(uri_string(),1);

		$this->layout->view('expensa/list');
	}

	public function expensa_update($primary_key_value){
		$this->rat->log(uri_string(),1);
	...
	public function expensa_delete($primary_key_value){
		$this->rat->log(uri_string(),1);
	...
	private function load_data_table($data)
	{
		if (count($data) == 0){
			return TRUE;
		}
	...
	public function cuenta_corriente_create() {
		$this->rat->log(uri_string(),1);
		if(!empty($_POST)){
			$data['unidad_id'] = $this->input->post('unidad_id',true);
			...
			if(!empty($_FILES['file_1']['name'])){
				...
				$config['upload_path'] = BASEPATH.'../upload/cuenta_corriente/';
				...
				$filename =  $data['user_id']."_".date("Y_m_d_H_i_s").
				'_expensa_'.'.'.strtolower($file_data['extension']);
			...
			if(!empty($_FILES['file_2']['name'])){
				...
				if($this->upload($config,'file_2')){ 
					$data['file_2'] = $filename;
				}
			}
			$this->Unidad_archivos_model->create($data);

			redirect('administrador/cuenta_corriente_list');
		}

		$data['unidades'] = $this->unidades_model->read(array('edificio_id'=>$this->edificio_id));
		$this->layout->view('cuenta_corriente/create',$data);
	}


	public function cuenta_corriente_read(){
		$this->rat->log(uri_string(),1);
		$limit = (isset($_POST['limit']))? $this->input->post('limit') : 50;
		$order_type = 'DESC';
		$order_by = 'unidad_archivos.id';
		$search = false;
		if($_POST){
			$limit = $this->input->post('limit',true);
			$order_by = $this->input->post('order_by',true);
		...
		$this->db->where('unidades.edificio_id',$this->edificio_id);
		$data['registers']  = $this->Unidad_archivos_model->read();
		$this->load->view('administrador/cuenta_corriente/read', $data);
	}

	public function cuenta_corriente_list(){
		$this->rat->log(uri_string(),1);
		$this->layout->view('cuenta_corriente/list');
	}

	public function cuenta_corriente_update($primary_key_value){
		$this->rat->log(uri_string(),1);

		$where['unidad_archivos.id'] = $primary_key_value;
		$where['unidades.edificio_id'] = $this->edificio_id;
		...
		if(!empty($_POST)){
			...
			$this->Unidad_archivos_model->update($data,['id'=>$primary_key_value]);

			redirect('administrador/cuenta_corriente_list');
		}
		$this->db->where($where);
		$rs = $this->Unidad_archivos_model->read();
		$data['cuenta'] = $rs->row();
		$data['unidades'] = $this->unidades_model->read(array('edificio_id'=>$this->edificio_id));
		$this->layout->view('cuenta_corriente/update', $data);
	}

	public function cuenta_corriente_delete($primary_key_value){
		$this->rat->log(uri_string(),1);

		$where['id'] = $primary_key_value;

		if(!$this->Unidad_archivos_model->delete($where)){
			redirect('administrador/edificios_error');
		}
		redirect('administrador/cuenta_corriente_list');
	...
	// create a new user
	private function create_user($identity,$password,$name,$edificio_id,$unidad_id)
    {
		$groups = array(PROPIETARIO);
		$additional_data = array(
		    'first_name' => $name,
		...
		if ($user_id = $this->ion_auth->register($identity, $password, $identity, $additional_data,$groups))
		{
			$this->edificios_model->add_edificios($user_id,[$edificio_id]);
			$unidad_id = array($unidad_id);
			$this->unidades_model->add_unidad($user_id,$unidad_id,PROPIETARIO);
		}
		return TRUE;
    }
