...
function get_ref($item_id){
  return base_url() . lang('lang', false) . '/' . lang($item_id . '_href');
}

if (!function_exists('get_country_by_ip'))
{
    function get_country_by_ip($client_ip = false)
    {
        ...
        if(!empty($client_ip) && !isset($_GET['nogeoip'])){
            
            $geo_ip = geoip_record_by_name($client_ip);
            /* echo '<pre style="display: none; opacity: 0" class="hide">';
            print_r($geo_ip);
            echo '</pre>'; */
            return strtolower($geo_ip['country_code']);
        }
...
// ------------------------------------------------------------------------
 ...
 // ------------------------------------------------------------------------
  ...
  function date_fix($hora_ingreso, $turno) 
  { 
    $jornal = $turno;
    ...
    return $hora_salida = $horas.":".$minutos.":00";
  } 


  function date_last_month_day() { 
      $month = date('m');
      $year = date('Y');
